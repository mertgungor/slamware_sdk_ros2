...
#include "../include/slamware_ros2/server_map_holder.hpp"
...
namespace slamware_ros_sdk {
    ...
    rpos::core::RectangleI ServerMapHolder::fillRosMapMsg(const rpos::core::RectangleI& reqIdxRect, nav_msgs::msg::OccupancyGrid& msgMap) const
    {
        ...
        msgMap.info.resolution = resolution_;
        msgMap.info.origin.position.x = resArea.x();
        msgMap.info.origin.position.y = resArea.y();
        msgMap.info.origin.position.z = 0.0;
        msgMap.info.origin.orientation.x = 0.0;
        msgMap.info.origin.orientation.y = 0.0;
        msgMap.info.origin.orientation.z = 0.0;
        msgMap.info.origin.orientation.w = 1.0;

        msgMap.info.width = 0;
        msgMap.info.height = 0;
        msgMap.data.clear();
        if (!sfIsCellIdxRectEmpty(resIdxRect))
        {
            msgMap.info.width = resIdxRect.width();
            msgMap.info.height = resIdxRect.height();
            const size_t destDatSize = msgMap.info.width * msgMap.info.height;
            msgMap.data.resize(destDatSize, -1);
            //
            if (!sfIsCellIdxRectEmpty(intersecRect))
            {
                ...
                for (int j = 0; j < intersecRect.height(); ++j)
                {
                    ...
                    for (int i = 0; i < intersecRect.width(); ++i)
                    {
                        ...
                        if (0 == tVal)
                            msgMap.data[tDestIdx] = 100;
                        else if (tVal <= 127)
                            msgMap.data[tDestIdx] = 0;
                        else if (127 < tVal)
                            msgMap.data[tDestIdx] = 100;
                        //
                        ++tSrcIdx;
    ...
    rpos::core::RectangleI ServerMapHolder::fillRosMapMsg(const rpos::core::RectangleF& reqArea, nav_msgs::msg::OccupancyGrid& msgMap) const
    {
    ...
    rpos::core::RectangleI ServerMapHolder::fillRosMapMsg(nav_msgs::msg::OccupancyGrid& msgMap) const
    {
